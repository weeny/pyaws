...
def setAssociateTag(associate_tag=None):
    """Set AWS Associate Tag Required since Oct 2011
        AWS won't let you search without an associate tag
        """

    global ASSOCIATE_TAG
    for get in __associateTags:
        rc = get(associate_tag)
        if rc: 
            ASSOCIATE_TAG = rc;
            return;
    raise NoAssociateTag, ("Please get an associate tag from  http://www.amazon.com/webservices")

def getAssociateTag():
    """Get Associate Tag;
    if no associate tag is known, NoAssociateTag is raised.
    """
